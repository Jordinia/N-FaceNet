...
def create_token(token):
    ...
    query = """
    INSERT INTO RegistrationToken (token, employee_id, created_date, expired_date) 
    VALUES (%s, %s, %s, %s)
    """
...
def get_tokens():
    ...
    cursor.execute("SELECT * FROM RegistrationToken")
...
def get_token_by_id(token_id):
    ...
    cursor.execute("SELECT * FROM RegistrationToken WHERE token_id = %s", (token_id,))
...
def get_token_by(**conditions):
    ...
    query = f"SELECT * FROM RegistrationToken WHERE {where_clause}"
...
def delete_token_by_id(token_id):
    ...
    cursor.execute("DELETE FROM RegistrationToken WHERE token_id = %s", (token_id,))
    connection.commit()
